...
static void* pdb_jg_read(int fd, const struct PDB_JG_HEADER* pdb, const WORD* block_list, int size)
{
    ...
    for (i = 0; i < nBlocks; i++)
        if (pread(fd, buffer + i * pdb->block_size, pdb->block_size,
...
static BOOL pdb_jg_init(int fd, struct pdb_reader* reader)
{
    ...
    if (pread(fd, (void*)&reader->u.jg.header, sizeof(reader->u.jg.header), 0) != sizeof(reader->u.jg.header)) return FALSE;
    reader->fd = fd;
    ...
    ret = pread(fd, blocks, size_blocks,
                offsetof(struct PDB_JG_HEADER, toc_block)) == size_blocks;
...
static void* pdb_ds_read(int fd, const struct PDB_DS_HEADER* header, const UINT *block_list, unsigned int size)
{
    ...
    for (i = 0; i < nBlocks; i++)
        if (pread(fd, buffer + i * header->block_size, header->block_size,
...
static BOOL pdb_ds_init(int fd, struct pdb_reader* reader)
{
    ...
    if (pread(fd, (void*)&reader->u.ds.header, sizeof(reader->u.ds.header), 0) != sizeof(reader->u.ds.header)) return FALSE;
    reader->fd = fd;
