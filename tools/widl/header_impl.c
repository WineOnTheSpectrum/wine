...
static void write_widl_using_macros( const type_t *iface )
{
    ...
    put_str( indent, "#define WIDL_impl_forwards_%s      WIDL_impl_forwards_%s\n", name, iface->c_name );
    put_str( indent, "#define WIDL_impl_each_forwards_%s WIDL_impl_each_forwards_%s\n", name, iface->c_name );

    put_str( indent, "#endif /* %s */\n\n", macro );
    free( macro );
}

static void write_widl_impl_macros_vtable_methods( const type_t *iface, const type_t *top_iface )
{
    const statement_t *stmt;

    if (type_iface_get_inherit( iface )) write_widl_impl_macros_vtable_methods( type_iface_get_inherit( iface ), top_iface );
...
static void write_widl_impl_macros( const type_t *iface )
{
    ...
    if (uuid)
    {
        ...
        put_str( indent, "    { \\\n" );
        write_widl_impl_macros_vtable_methods( iface, iface );
        put_str( indent, "    };\n" );
        put_str( indent, "\n" );

        put_str( indent, "#define WIDL_impl_%s( type, prefix, ... ) \\\n", iface->c_name );
        put_str( indent, "    WIDL_impl_from_%s( type ) \\\n", iface->c_name );
        if (object) put_str( indent, "    WIDL_impl_IUnknown_methods( type, %s, prefix, __VA_ARGS__ ) \\\n", name );
        if (inspectable) put_str( indent, "    WIDL_impl_IInspectable_methods( type, %s, prefix ) \\\n", name );
        put_str( indent, "    WIDL_impl_each_forwards( type, %s, &object->%s_iface, __VA_ARGS__ )\n", name, name );

        put_str( indent, "#define WIDL_impl_outer_%s( type, prefix, ... ) \\\n", iface->c_name );
        put_str( indent, "    WIDL_impl_from_%s( type ) \\\n", iface->c_name );
        if (object) put_str( indent, "    WIDL_impl_IUnknown_methods( type, %s, prefix, __VA_ARGS__ ) \\\n", name );
        if (inspectable) put_str( indent, "    WIDL_impl_IInspectable_methods( type, %s, prefix ) \\\n", name );
        put_str( indent, "    WIDL_impl_each_forwards( type, %s, object->outer, __VA_ARGS__ )\n", inspectable ? "IInspectable" : "IUnknown" );

        put_str( indent, "#define WIDL_impl_static_%s( type, prefix, ... ) \\\n", iface->c_name );
        put_str( indent, "    WIDL_impl_from_%s( type ) \\\n", iface->c_name );
        if (object) put_str( indent, "    WIDL_impl_static_IUnknown_methods( type, %s, prefix, __VA_ARGS__ ) \\\n", name );
        if (inspectable) put_str( indent, "    WIDL_impl_IInspectable_methods( type, %s, prefix ) \\\n", name );
        put_str( indent, "    WIDL_impl_each_forwards( type, %s, &object->%s_iface, __VA_ARGS__ )\n", name, name );
        put_str( indent, "\n" );

        put_str( indent, "#define WIDL_impl_forwards_%s( type, base, expr, prefix ) \\\n", iface->c_name );
        put_str( indent, "    WIDL_impl_from_%s( type ) \\\n", iface->c_name );
        if (object) put_str( indent, "    WIDL_impl_IUnknown_forwards( type, %s, base, expr, prefix ) \\\n", name );
        if (inspectable) put_str( indent, "    WIDL_impl_IInspectable_forwards( type, %s, base, expr, prefix ) \\\n", name );
        put_str( indent, "\n" );

        put_str( indent, "#define WIDL_impl_each_forwards_%s( type, base, expr, prefix, X, Y, ... ) \\\n", iface->c_name );
        put_str( indent, "    WIDL_impl_forwards_%s( type, base, expr, prefix ) \\\n", name );
        put_str( indent, "    WIDL_impl_each_forwards_ ## X( type, base, expr, Y, __VA_ARGS__ )\n" );
        put_str( indent, "\n" );
    }
}
...
static void write_interface( const type_t *iface )
{
    ...
    {
        put_str( indent, "#define WIDL_impl_each_forwards( type, base, expr, X, Y, ... ) WIDL_impl_each_forwards_ ## X( type, base, expr, Y, __VA_ARGS__ )\n" );
        put_str( indent, "#define WIDL_impl_each_forwards_END( type, base, expr, prefix, ... )\n" );
        put_str( indent, "\n" );
        ...
        put_str( indent, "#define WIDL_impl_IUnknown_Release( type, name, prefix ) WIDL_impl_IUnknown_Release_( type, name, prefix, type ## _from_ ## name )\n" );
        put_str( indent, "#define WIDL_impl_IUnknown_Release_( type, name, prefix, impl_from ) \\\n" );
        ...
        put_str( indent, "        return ref; \\\n" );
        put_str( indent, "    }\n" );
        put_str( indent, "#define WIDL_impl_static_IUnknown_Release( type, name, prefix ) \\\n" );
        put_str( indent, "    static ULONG WINAPI prefix ## _Release( name *iface ) \\\n" );
        put_str( indent, "    { \\\n" );
        ...
        put_str( indent, "#define WIDL_impl_IUnknown_methods( type, name, prefix, ... ) \\\n" );
        put_str( indent, "    WIDL_impl_IUnknown_QueryInterface_( type, name, prefix, type ## _from_ ## name, __VA_ARGS__ ) \\\n" );
        put_str( indent, "    WIDL_impl_IUnknown_AddRef_( type, name, prefix, type ## _from_ ## name ) \\\n" );
        put_str( indent, "    WIDL_impl_IUnknown_Release_( type, name, prefix, type ## _from_ ## name )\n" );
        put_str( indent, "\n" );
        put_str( indent, "#define WIDL_impl_static_IUnknown_methods( type, name, prefix, ... ) \\\n" );
        put_str( indent, "    WIDL_impl_IUnknown_QueryInterface_( type, name, prefix, type ## _from_ ## name, __VA_ARGS__ ) \\\n" );
        put_str( indent, "    WIDL_impl_static_IUnknown_AddRef( type, name, prefix ) \\\n" );
        put_str( indent, "    WIDL_impl_static_IUnknown_Release( type, name, prefix )\n" );
        put_str( indent, "\n" );
    ...
    {
        put_str( indent, "#define WIDL_impl_IInspectable_methods( type, name, prefix ) WIDL_impl_IInspectable_methods_( type, name, prefix, type ## _from_ ## name )\n" );
        put_str( indent, "#define WIDL_impl_IInspectable_methods_( type, name, prefix, impl_from ) \\\n" );
        ...
        put_str( indent, "#define WIDL_impl_IInspectable_forwards( type, name, base, expr, prefix ) WIDL_impl_IInspectable_forwards_( type, name, base, expr, prefix, type ## _from_ ## name )\n" );
        put_str( indent, "#define WIDL_impl_IInspectable_forwards_( type, name, base, expr, prefix, impl_from ) \\\n" );
        put_str( indent, "    static HRESULT WINAPI prefix ## _GetIids( name *iface, ULONG *count, IID **iids ) \\\n" );
        put_str( indent, "    { \\\n" );
        put_str( indent, "        struct type *object = impl_from( iface ); \\\n" );
