...
/* grow a COMCTL32_UnicodeBuffer if needed so it can hold at least minWChars WCHARs */
/* return FALSE means failed (unicodeBuffer still valid but too small) */
/* return TRUE means successful */
static BOOL COMCTL32_AdjustUnicodeBuffer(COMCTL32_UnicodeBuffer *unicodeBuffer, DWORD minWChars)
{
    WCHAR *address;
    DWORD minSizeInBytes;

    if (minWChars > ~(DWORD)0 / sizeof(WCHAR))
        return FALSE;
...
LRESULT COMCTL32_ForwardNotifyToAnsiWindow(HWND hwndNotify, NMHDR *hdr, COMCTL32_UnicodeBuffer *unicodeBuffer)
{
    ...
    case CBEN_GETDISPINFOW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmcbe->ceItem.mask, CBEIF_TEXT, &nmcbe->ceItem.pszText,
    ...
    case CBEN_DRAGBEGINW:
    {
        ...
        WideCharToMultiByte(CP_ACP, 0, nmdbW->szText, ARRAY_SIZE(nmdbW->szText), nmdbA.szText, ARRAY_SIZE(nmdbA.szText),
                            NULL, FALSE);
        return SendMessageW(hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)&nmdbA);
    }
    case CBEN_ENDEDITW:
    {
        ...
        WideCharToMultiByte(CP_ACP, 0, nmedW->szText, ARRAY_SIZE(nmedW->szText), nmedA.szText, ARRAY_SIZE(nmedA.szText),
                            NULL, FALSE);
        return SendMessageW(hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)&nmedA);
    }
    /* Date and Time Picker */
    case DTN_FORMATW:
    {
        ...
        ret = SendMessageW(hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)nmdtf);
        ...
        if (nmdtf->pszDisplay)
        {
            ...
            if (!COMCTL32_AdjustUnicodeBuffer(unicodeBuffer, textLength)) return ret;
            MultiByteToWideChar(CP_ACP, 0, (LPCSTR)nmdtf->pszDisplay, -1, unicodeBuffer->address, textLength);
            if (nmdtf->pszDisplay != nmdtf->szDisplay)
                nmdtf->pszDisplay = unicodeBuffer->address;
            else
            {
                textLength = min(textLength, ARRAY_SIZE(nmdtf->szDisplay));
                memcpy(nmdtf->szDisplay, unicodeBuffer->address, textLength * sizeof(WCHAR));
            }
        }
    ...
    case DTN_FORMATQUERYW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, (WCHAR **)&nmdtfq->pszFormat, NULL, CONVERT_SEND);
    }
    case DTN_WMKEYDOWNW:
    {
        NMDATETIMEWMKEYDOWNW *nmdtkd = (NMDATETIMEWMKEYDOWNW *)hdr;
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, (WCHAR **)&nmdtkd->pszFormat, NULL, CONVERT_SEND);
    }
    case DTN_USERSTRINGW:
    {
        NMDATETIMESTRINGW *nmdts = (NMDATETIMESTRINGW *)hdr;
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, (WCHAR **)&nmdts->pszUserString, NULL, CONVERT_SEND);
    }
    /* Header */
    ...
    case HDN_TRACKW:
    {
        ...
        if (!nmh->pitem) return SendMessageW(hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)hdr);
    ...
    case HDN_GETDISPINFOW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmhddi->mask, HDI_TEXT, &nmhddi->pszText, &nmhddi->cchTextMax,
    ...
    case LVN_SETDISPINFOW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmlvdi->item.mask, LVIF_TEXT, &nmlvdi->item.pszText,
    ...
    case LVN_GETINFOTIPW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, &nmlvgit->pszText, &nmlvgit->cchTextMax,
    ...
    case LVN_ODFINDITEMW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmlvfi->lvfi.flags, LVFI_STRING | LVFI_SUBSTRING,
    ...
    case TBN_GETBUTTONINFOW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, &nmtb->pszText, &nmtb->cchText,
                                            SEND_EMPTY_IF_NULL | CONVERT_SEND | CONVERT_RECEIVE);
    }
    case TBN_GETINFOTIPW:
    {
        NMTBGETINFOTIPW *nmtbgit = (NMTBGETINFOTIPW *)hdr;
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, &nmtbgit->pszText, &nmtbgit->cchTextMax, CONVERT_RECEIVE);
    }
    /* Tooltip */
    case TTN_GETDISPINFOW:
    {
        ...
        ret = SendMessageW(hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)&nmttdiA);
        ...
        else if (!IS_INTRESOURCE(nmttdiA.lpszText))
        {
            ...
            {
                if (!COMCTL32_AdjustUnicodeBuffer(unicodeBuffer, size)) return ret;
                MultiByteToWideChar(CP_ACP, 0, nmttdiA.lpszText, -1, unicodeBuffer->address, size);
                nmttdiW->lpszText = unicodeBuffer->address;
    ...
    case TVN_ITEMEXPANDINGW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmtv->itemNew.mask, TVIF_TEXT, &nmtv->itemNew.pszText, NULL,
                                            CONVERT_SEND);
    }
    case TVN_DELETEITEMW:
    {
        NMTREEVIEWW *nmtv = (NMTREEVIEWW *)hdr;
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmtv->itemOld.mask, TVIF_TEXT, &nmtv->itemOld.pszText, NULL,
                                            CONVERT_SEND);
    }
    ...
    case TVN_ENDLABELEDITW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, &nmtvdi->item.mask, TVIF_TEXT, &nmtvdi->item.pszText,
    ...
    case TVN_SELCHANGEDW:
    {
        ...
        if (!((nmtv->itemNew.mask | nmtv->itemOld.mask) & TVIF_TEXT))
            return SendMessageW(hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)hdr);
    ...
    case TVN_GETINFOTIPW:
    {
        ...
        return COMCTL32_SendConvertedNotify(hwndNotify, hdr, NULL, 0, &nmtvgit->pszText, &nmtvgit->cchTextMax, CONVERT_RECEIVE);
    }
    }
