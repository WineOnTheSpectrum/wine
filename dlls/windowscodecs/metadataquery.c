        ...
        GUID *guid, unsigned int *index, IUnknown *object)
{
    ...
    *index = 0;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        enumerator->guids.entries[enumerator->guids.count].index = 0;
        ...
        LPOLESTR *strings, ULONG *fetched)
{
    ...
    *fetched = 0;
    ...
    else if (enumerator->metadata_enumerator)
    {
        ...
        if (SUCCEEDED(hr = IWICEnumMetadataItem_Next(enumerator->metadata_enumerator, count, schemas, ids, NULL, fetched)))
        {
            for (ULONG i = 0; i < *fetched; ++i)
            {
    ...
    return *fetched < count ? S_FALSE : S_OK;
}
...
static void string_enumerator_init(struct string_enumerator *object, IEnumUnknown *object_enumerator,
        IWICEnumMetadataItem *metadata_enumerator);
...
static HRESULT WINAPI string_enumerator_Clone(IEnumString *iface, IEnumString **out)
{
    ...
    *out = NULL;
    ...
    string_enumerator_init(object, object_enumerator, metadata_enumerator);
...
static HRESULT string_enumerator_create(struct query_handler *handler, IEnumString **enum_string)
{
    IWICEnumMetadataItem *metadata_enumerator = NULL;
    IEnumUnknown *object_enumerator = NULL;
    struct string_enumerator *object;
