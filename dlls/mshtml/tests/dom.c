...
#undef CHARSETS
    ...
    for(doc_charset = utf8; doc_charset <= utf16; doc_charset++) {
        for(css_charset = utf8; css_charset <= utf16_bom; css_charset++) {
            notif_doc = doc = create_document();
            if(!doc)
                return;
            doc_complete = FALSE;
            ...
            if(doc_charset == utf16)
                size = wsprintfW(buf, L"<!DOCTYPE html>\n<html><head><link href=\"res://%s/%s.css\" rel=\"stylesheet\"></head><body></body></html>", module_pathW, charsetsW[css_charset]) * sizeof(WCHAR);
            else
                size = sprintf(buf, "<!DOCTYPE html>\n<html><head><link href=\"res://%s/%s.css\" rel=\"stylesheet\"></head><body></body></html>", module_path, charsets[css_charset]);
            ...
            ok(!wcscmp(V_BSTR(&var), L"rgb(222, 173, 184)"), "[%s:%s] backgroundColor = %s\n", charsets[doc_charset], charsets[css_charset], wine_dbgstr_w(V_BSTR(&var)));
